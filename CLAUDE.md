...
- Vitest を使用
- `describe`, `it`, `expect` を `vitest` からインポート
...
- **describe**: 関数名 → カテゴリ（期待結果を含む）の階層構造
  - 例: `describe("isValidInput", () => { describe("許可される入力（true）", () => { ... }) })`
- **it**: 日本語で「〜を許可」「〜を禁止」「〜を除去」「〜を保持」など動作を明記
  - 例: `it("半角英字（小文字）を許可", () => { ... })`
  - 例: `it("HTMLタグを禁止", () => { ... })`
- **コメント**: `// ===== カテゴリ名 =====` でテストケースをグループ化
